...
**Current Lesson:** 5.6 complete (demo lesson — Claude wrote, user read)
**Last Session Date:** 2026-05-10
**Notes:** WASI sandbox demo. New `cmd/tascript-wasm/main.go` (~50 LOC, reads script from stdin, optional candles via WASI fs mount) compiles with `GOOS=wasip1 GOARCH=wasm` to a 2.9 MB `tascript.wasm`. New `cmd/sandbox/main.go` (~110 LOC) embeds wazero v1.11.0, configures `WithMemoryLimitPages(256)` = 16 MB cap and `WithCloseOnContextDone(true)` + 5s deadline, mounts host's `.` at guest's `/` for `data.csv` access, pipes script stdin / stdout / stderr through. `examples/demo.tas` produces identical output via both `go run . examples/demo.tas` and `go run ./cmd/sandbox examples/demo.tas` — the equivalence proof. Runaway-recursion script demonstrated the structural memory cap firing: Go's WASI runtime tried to grow the goroutine stack past 8 MB, wazero denied the memory.grow at the 16 MB cap, Go's runtime panicked OOM inside the module, host caught exit code 2. Subtle behavior worth flagging: when Go-WASI hits the memory cap, the trap surfaces as a Go runtime panic, not a clean wazero memory-grow error. Other language backends (Rust, AssemblyScript) would surface it more cleanly. `tascript.wasm` gitignored.
...
- [x] **5.6 — (Stretch) WASM Sandbox Runner**
...
| 22      | 2026-05-10 | 5.6             | WASM sandbox demo. Demo lesson — Claude implemented, user observed. `cmd/tascript-wasm/main.go` (~50 LOC) is the WASI build target; `cmd/sandbox/main.go` (~110 LOC) is the wazero host with 16 MB memory cap + 5s deadline + WASI fs mount. Build: `GOOS=wasip1 GOARCH=wasm go build -o tascript.wasm ./cmd/tascript-wasm` (2.9 MB output). Run: `go run ./cmd/sandbox examples/demo.tas` — identical output to native binary, equivalence proven. Spontaneous structural-cap demo when running an infinite-recursion script: Go WASI runtime tried to grow goroutine stack past 8 MB, wazero denied at 16 MB cap, Go panicked OOM, host caught exit code 2. Quirk noted: Go-WASI surfaces memory-cap traps as runtime panics rather than clean wazero errors — different from how Rust/AssemblyScript would surface them. Custom host ABI (Stage 4 multi-language) deliberately deferred — could be a future 5.7. `tascript.wasm` added to .gitignore; wazero v1.11.0 added to go.mod. |
