...
Suggested order (re-sequenced 2026-07-22 — #6/#8/#9/#10 now shipped):
**#7 first** (runtime input binding — finishes the input/output arc; makes a realistic
script — struct input, condition, emit — runnable end-to-end for the first time, and is the
fixture #13 should be designed around). Then the P1 hole-plugging batch #2/#3/#5 (same
disease: silent `BadExpr` holes reachable from valid parses, failing without diagnostics).
Then #14 before any external tooling consumes diagnostics. #13 (Engine/host API rework) last,
informed by the `Emitted()` + `EvalRun(inputs)` stopgaps being used in anger. Beyond this
