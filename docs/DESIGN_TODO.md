...
|---|------|------|---------|---------|
| 1 | P1 | evaluator/registry | `5 % 0` panics the whole host process (`leftInt % rightInt`, std_math.go:131); no recover anywhere | Verified: process crash from a valid script. Same error family, three inconsistent behaviors: int `%` 0 panics, int `/` 0 → `+Inf` silently (float conversion), float `%` 0.0 → `NaN` silently. A stopgap (guard or recover at the eval boundary) is independent of the #10 protocol decision and should not wait for it |
...
Suggested order: stopgap #1 first (trivial, stops host crashes). Then #7 (finishes the
...
- `%` / `/` by zero, `math.sqrt(-1)` — would have caught tracker #1.
