...
| 6 | P2 | all | `state.*` — implement the locked design (2026-07-05, spec §3.2 item 4 / §4.3 rewritten) | **Design locked — plain persistent typed variables, NOT lookback/ring-buffer** (lookback model explicitly rejected). `state name: Type [= const-expr]` top-level decl; access `state.name` read/write in Init/Run; no history, no `[n]`, no bracket access. Static rules: undeclared access → STATE_UNDECLARED; initializer type must match decl (Int→Float coerces); no initializer → **definite assignment** required in Init — every execution path must assign; if/else both-branches counts, lone if doesn't; conservative walker in `definitelyAssignedState` (STATE_UNINITIALIZED); scalar entry types only (record type → diag). Initializer domain = const domain, enforced by resolve order (consts → state decls → inputs/outputs; `state` itself not referable until after state decls resolve). Load order: consts → state initializers → Init() — same phase work as #9, one design. Runtime: persistent store on the evaluator surviving across EvalRun calls; Init writes land there. Parser: new `state` decl (fold into the input/output decl helper — see style backlog on their copy-paste) |
